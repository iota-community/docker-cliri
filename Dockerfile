...
MAINTAINER info@antonionardella.it
...
COPY --from=local_stage_build /cliri/target/cliri*.jar /cliri/target/
COPY cliri/docker/entrypoint.sh /

# Java related options. Defaults set as below
ENV JAVA_OPTIONS="-XX:+UnlockExperimentalVMOptions -XX:+DisableAttachMechanism -XX:InitiatingHeapOccupancyPercent=60 -XX:G1MaxNewSize$
ENV JAVA_MIN_MEMORY 2G
ENV JAVA_MAX_MEMORY 4G
# Additional custom variables. See DOCKER.md for details
ENV DOCKER_IRI_JAR_PATH "/cliri/target/cliri*.jar"
